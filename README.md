...
The final hurdle I had to overcome was one of misdirection. Nextflow relies heavily upon symlinks when dealing with local cluster filesystems to prevent excessive copying of files among work directories. As a result, a channel that is generated by `process_a()`, emitted and consumed by `process_b()`, and modified and consumed by `process_c()` may be 
